  ...
  return (
    <>
      ...
      <button
        ...
        className={`md:hidden fixed top-6 left-6 z-40 p-2 rounded-lg ${isDark ? 'bg-orange-600 text-white' : 'bg-orange-100 text-orange-600'}`}
      >
        {open ? <X className="w-7 h-7" /> : <Menu className="w-8 h-8" />}
      </button>

      {/* Sidebar */}
      <aside className={`fixed left-0 top-0 h-screen w-64 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-r transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 z-30 pt-20 md:pt-0 flex flex-col`}>
